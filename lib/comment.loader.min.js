...
(function (root, doc) {
  ...
  function setLocalStorageItem(key, data, expireSecond) {
    ...
    var expireTime = expireSecond ? nowTime + expireSecond * 1000 : ''; // eslint-disable-line
    var value = { data: data };
  ...
  var Loader = (_temp = _class = function () {
    ...
    _createClass(Loader, [{
      ...
      value: function getFilePaths() {
        var cacheData = getLocalStorageItem(this.loaderPath);
        return cacheData;
      }
    }, {
      key: 'saveFilePaths',
    ...
    this.removeLoadedScript = function (name) {
      var script = doc.getElementById(name);
    ...
    this.randomString = function () {
      var res = parseFloat('0.' + Math.floor(new Date().getTime() / 1000 / _this.dateStep()) * 9999);
    ...
    this.loadScripts = function (urls, callback) {
      ...
      var _loadScript = function _loadScript() {
        ...
        var src = url.indexOf('//') === -1 && options.staticHost ? options.staticHost + url : url;
    ...
    this.updateFilePath = function (callback) {
      ...
      var url = mapPath.indexOf('//') === -1 && staticHost ? staticHost + mapPath : mapPath;
      that.ajax({
        url: url,
        success: function success(res) {
          if (res) {
            ...
            if (result.data) {
              ...
              if (isArray(result)) {
                urls = [].concat(_toConsumableArray(result));
    ...
    this.run = function (callback) {
      ...
      var paths = that.getFilePaths();
      if (paths && paths.data && !options.async) {
        that.loadScripts(paths.data, function () {
...
__webpack_require__(/*! ./loader.v2 */ 0);

window.setCommentEnv = function (env) {
  var pkgName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'comment-sdk.commentsdk';

  var listId = pkgName + '.' + env;
  var apiPath = '//napi.uc.cn/3/classes/sdk_loader_maps/lists/' + listId;
  var apiQuery = ['_app_id=zdl', '_fetch=1', '_size=1'];
  return {
